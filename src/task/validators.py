...
class IdValidator:
    """
    Дескриптор для валидации идентификатора: проверяет,
    что ID является целым неотрицательным числом.
    """
...
class EnumValidator:
    """
    Дескриптор для валидации Enum: проверяет соответствие данных указанному enum,
...
class DescriptionValidator:
    """
    Дескриптор для валидации описания: проверяет тип строки и длину (от 1 до 200 символов).
    """
    def __init__(self) -> None:
        self._name: Optional[str] = None
...
class CreatedAtValidator:
    """
    Дескриптор для валидации даты создания: разрешает установку значения только один раз.
    """
    def __init__(self) -> None:
        self._name: Optional[str] = None
